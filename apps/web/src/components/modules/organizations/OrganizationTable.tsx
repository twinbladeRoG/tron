...
import { useOrganizations } from '@/apis/queries/organizations.queries';
  ...
  const columns = useMemo(
    () => [
      ...
      columnHelper.display({
        ...
        cell: (info) => <OrganizationTableActions organization={info.row.original} />,
      }),
    ],
    []
  );

  const data = useMemo(() => organizations.data?.data ?? [], [organizations.data]);

  // eslint-disable-next-line react-hooks/incompatible-library
  const table = useReactTable({
    data: data,
    columns,
    rowCount: organizations.data?.pagination.total_count ?? 0,

...
const AddPolicy: React.FC<AddPolicyProps> = ({ className, onSubmit }) => {
  ...
  const handleAutoGenerateDescription = () => {
    const subjectType = form.getValues('subjectType');
    const subjectId = form.getValues('subjectId');
    const objectType = form.getValues('objectType');
    const objectId = form.getValues('objectId');
    const action = form.getValues('action');
    const effect = form.getValues('effect');

    if (!subjectId || !objectId) return;

    const capitalize = (str: string) => str.charAt(0).toUpperCase() + str.slice(1);
    const subject = subjectType === 'user' ? subjectId : subjectId.toUpperCase();

    const description = `${capitalize(effect)} ${subjectType} ${subject} to ${action} ${objectType} ${objectId}`;
  ...
  return (
    <form className={className} onSubmit={handleSubmit}>
      ...
      <div className="mb-4 flex justify-end">
        <Button
          size="compact-md"
          leftSection={<Icon icon="solar:star-shine-bold" />}
          variant="light"
          type="button"
